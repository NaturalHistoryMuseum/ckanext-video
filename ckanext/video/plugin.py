...
class VideoPlugin(p.SingletonPlugin):
    ...
    def setup_template_variables(self, context, data_dict):
        ...
        video_url = data_dict[u'resource_view'].get(u'video_url') or data_dict[u'resource'].get(u'url')

        # Is this a youtube video?
        if u'youtube.com' in video_url:
            # If this is a youtube link, replace with a URL for embeddable video
            match = re.search(video_provider_patterns[u'youtube_link'], video_url, re.IGNORECASE)
            if match:
